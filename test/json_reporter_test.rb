...
class JsonReporterTest < Minitest::Test
  ...
  def mutation_at(token, replacement, operator)
    off = SRC.index(token)
    Brutus::Mutation.new(start_offset: off, end_offset: off + token.length,
  ...
  def subject
    ...
    Brutus::Subject.new(file: FILE, namespace: ["Pricing"], name: :total,
                        singleton: false, def_node: def_node)
  end

  def survivor
    Brutus::Result.survived.with(subject: subject, mutation: mutation_at(">=", ">", :comparison))
  end

  def render(results)
    out = StringIO.new
    Brutus::Reporter.new(Brutus::AggregateResult.new(results), { FILE => SRC })
                    .report(out: out, err: StringIO.new, format: "json")
    JSON.parse(out.string)
  end

  def test_valid_json_with_summary_and_score
    doc = render([Brutus::Result.killed, survivor])
  ...
  def test_empty_arrays_when_nothing_survives
    doc = render([Brutus::Result.killed])
  ...
  def test_survivors_sorted_by_file_line_operator
    a = Brutus::Result.survived.with(subject: subject, mutation: mutation_at("100", "0", :literal_mutation))
    b = Brutus::Result.survived.with(subject: subject, mutation: mutation_at(">=", ">", :comparison))
  ...
  def test_output_to_file_keeps_stdout_clean
    Dir.mktmpdir do |dir|
      ...
      out = StringIO.new
      err = StringIO.new
      Brutus::Reporter.new(Brutus::AggregateResult.new([survivor]), { FILE => SRC })
