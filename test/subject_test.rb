...
class SubjectTest < Minitest::Test
  def def_node(source)
    Brutus::Parser.parse_string(source).value.statements.body.first
  end

  def test_qualified_name_instance_method
    s = Brutus::Subject.new(file: "f.rb", namespace: %w[Billing Invoice],
  ...
  def test_qualified_name_top_level
    s = Brutus::Subject.new(file: "f.rb", namespace: [],
